...
function LectureRow({ lec, onToggle }: { lec: Lecture; onToggle: (id: string, done: boolean) => void }) {
  ...
  return (
    <div className="border border-black/10 rounded-xl bg-ink-2 p-4 flex items-center gap-4">
      <button
        ...
        className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center text-xs transition-colors ${
          done ? "bg-verified border-verified text-deep" : "border-black/15 text-transparent"
        }`}
      ...
      <span className="text-[10px] font-body text-mute flex-shrink-0">{lec.recordedOn}</span>
    </div>
  );
}
...
function LecturesBody() {
  ...
  if (!lectures) return <p className="text-mute text-sm font-body">Loading recorded lectures…</p>;
  ...
  return (
    <div>
      <p className="text-[10px] uppercase tracking-[0.2em] text-gold font-body mb-2">Recorded Lectures</p>
        ...
        {subjects.map((s) => (
          <button
            ...
            className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${
              filter === s ? "bg-gold text-deep border-gold" : "border-black/10 text-mute hover:text-deep"
            }`}
          >
            {s}
          </button>
